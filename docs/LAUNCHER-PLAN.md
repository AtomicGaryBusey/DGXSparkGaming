...
> *"The narrative should live in the README. The whole corpus of raw data doesn't need to live
> within the narrative."*

So the boundary is **content type**, not "generated vs hand-written":

| | Lives in | Sole source of truth for | Hand-authored? |
|---|---|---|---|
| **Narrative** — diagnoses, retractions, the story of how something was found out, why a thing works, setup guidance | `README.md` | the *argument* | **Yes, always. Never generated.** |
| **Data** — titles, sources, installs, runs, artifacts, claims, signatures, measurements | `sgl.db` | the *facts* | No. Written through the CLI/GUI only. |

This is better than the export-mirroring scheme the architecture originally proposed, and it
retires it. The earlier worry — that generating README.md from the database would flatten the
prose that is this project's most valuable content — simply does not arise, because README.md
stops being generated at all. It also retires the "which one wins in a conflict" question: they
cannot conflict, because they hold different kinds of thing.

**What moves out of README.md.** The results tables — 161 rows today. They are data. README keeps
the narrative that *explains* results and loses the ledger that *enumerates* them.

**What this obliges us to build.** The repo is public, and a stranger who does not run the
launcher must still be able to read the results. So the database owes the repo one **generated**
view — `docs/RESULTS.md`, regenerated by `sgl export`, marked as generated, never hand-edited.
That file is data, so generating it costs nothing that matters. README links to it.

**Durability, which is a separate concern from truth.** A binary SQLite file is a poor thing to
have as the only copy of work that took 300 GB of installs to produce. `sgl export --sql` writes
a plain-text dump committed on a schedule. That is a **backup**, not a second source of truth, and
it is never read back except to recover.

**Git history stays meaningful**, which was the strongest objection to a database-only design.
Every judgement — every claim, correction and retraction — still lands in README.md as reviewable
prose in a commit with a message. Only the ledger moves. The audit trail this project actually
relies on is the narrative one, and it is untouched.

**Migration.** The 161 existing rows import as claims with `verifier_class='human_observed'` and
their real provenance, then get re-validated under instrumentation (AGB, 2026-09-08: previous
human-verified runs are to be re-done with proper tooling). README.md is then rewritten
narrative-only. That rewrite is a real piece of work on a 2,887-line file and belongs in M1, not
in a footnote.
