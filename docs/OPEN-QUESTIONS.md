...
| **Box64 writes the FSAVE tag word stack-relative, not physical — AND THIS IS THE id Tech 4 x87 FAILURE. Fixed by a rotate in `fpu_savenv`; verified `0xffc0`->`0x03ff` with no regression** | `evidence/2026-09-08-box64-x87-tagword/` | high — caught in two games and graded by the probe suite |
