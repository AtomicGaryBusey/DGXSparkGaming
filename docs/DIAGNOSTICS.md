...
|---|---|---|
| `Access violation in steamclient_init` | proton.log | **Box64 only, root-caused 2026-09-08.** The exe is SteamStub-wrapped (a `.bind` section with the entry point inside it) so its stub loads `Steam.dll`; Proton redirects that to builtin `lsteamclient`, whose 32-bit unix half needs `libstdc++.so.6`, which imports four symbols missing from Box64's box32 wrapper table (`arc4random`, `strfromf128`, `strtof128`, `strtold`). `dlopen` fails under bind-now, `__wine_unixlib_handle` stays 0, and `call [eax+edx*4]` faults with `eax=0`. **Look for `Symbol ... not found, cannot apply R_386_JMP_SLOT` and `relocating Plt symbols in elf libstdc++.so.6` earlier in the same log** — those are the cause; the AV is the symptom. FEX is unaffected. |
