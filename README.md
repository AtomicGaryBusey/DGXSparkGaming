...
| **Prey (2006)** | OpenGL ARB2 (id Tech 4 variant, Human Head), 32-bit. **LAUNCHES under Box64 with the four-symbol box32 fix (2026-09-08).** Engine reaches `Initializing OpenGL driver` -> `PIXELFORMAT 1 selected` -> **`creating GL context: succeeded`** -> `Initializing Session` -> `Common Initialization Complete`. The only thing left is a CD-key prompt, and that is a PATH problem rather than a licensing one: the engine reads `../base/preykey`, which resolves to `common/base/` when the cwd is the game root, while the file actually sits in `Prey 2006/base/preykey`. `tools/game-run.sh` gained `WORKDIR=` for exactly this — launch with cwd `<game>/base` and `../base/preykey` resolves correctly. **Under FEX the same build never gets a GL context at all** (`SetPixelFormat failed` x2), so this title is a clean demonstration that the translator, not the engine, is the variable. Without the box64 fix it dies far earlier, in `steamclient_init`. Evidence: `evidence/2026-09-08-steamclient-init-box64/`. The CD-key prompt is **not** a path problem (that theory was tried and was wrong — setting the cwd to the mod dir makes id Tech 4 search `base/base` and load no `.pk4`s at all). Steam writes the legacy key file when it launches a title through its own path, and Prey has only ever been launched directly here. Measured: `quake4key` line 1 = 21 chars (works), `STEAM_doomkey` = 7 chars, `preykey` = **0 chars**, an empty template Prey wrote itself. **Fix: launch Prey once from the Steam UI**, then direct launches work. **TODO:** confirm gameplay after that. |
