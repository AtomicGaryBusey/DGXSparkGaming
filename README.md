...
| **Daikatana** | OpenGL | Excellent — very high framerate, stable visuals, snappy loading | **id Tech 2** (Quake II engine), 2000. **32-bit x86** (`daikatana.exe` = PE32/Intel 80386) through **an unverified runtime** — see the warning below — the Cortex-X925/A725 cores have no 32-bit ARM support at all, so this is pure translation. Renderer is `ref_gl.dll` → Wine `opengl32.dll` → `winex11.drv` → native ARM64 GL: **no DXVK/Vulkan involved**, unlike almost everything else in this table. Steam runs `dxsetup.exe` on first launch (harmless; the game's backends are OpenGL/Glide/PowerVR and never need D3D). 13 `err:` lines in the Proton log, all benign. **⚠️ RUNTIME ATTRIBUTION UNVERIFIED (flagged 2026-09-08).** This row says "FEX's 32-bit path", but on 2026-09-08 a probe measured that FEX can set **0 of 320** pixel formats for any 32-bit Wine program, mapped window or not, while Box64 manages 320/320 — and `ref_gl.dll` imports exactly that path (`SetPixelFormat`, `ChoosePixelFormat`, `wglCreateContext`). Both statements cannot be true. The Daikatana runs predate `run.json`, so they carry **no `runtime_actual`** and no Proton log — only `gpu.csv` and `launch.log` — which is precisely the era this log mis-attributed a Box64 run to FEX. **Do not cite this row as evidence that 32-bit OpenGL works under FEX.** Resolve with `tools/ab-runtime.sh 242980`. |
