...
| **Prey (2006)** | OpenGL ARB2 (id Tech 4 variant, Human Head), 32-bit. **PLAYABLE 2026-09-08 under Box64 with both local fixes** — past the intro cutscene into real gameplay, confirmed by a human at the keyboard. Needs (a) the four missing box32 libc wrappers, without which it dies in `steamclient_init`, and (b) a CD key, which Steam only writes when the title is launched from the Steam UI at least once. **Under FEX it never gets a GL context at all** (`SetPixelFormat failed` x2), so this title is the cleanest demonstration in this log that the TRANSLATOR is the variable, not the engine. **TODO:** mouse input behaves oddly, the same as Quake 4. Both games also log an identical recurring fault — `c0000005` at `ntdll.so+0x71f0` reading `0x7c`, 92x in Prey and 41x in Quake 4 — but **that fault is NOT the input path and may be unrelated to the mouse**: a DirectInput probe (`tools/probes/dinput/`) walks the full DI8 enumerate/create/acquire/read sequence with `DI_OK` at every step and zero faults, and the faults arrive in periodic bursts 60-80 s apart rather than per frame. Both games ship PunkBuster, whose scan cadence fits, but that is untested. See `docs/OPEN-QUESTIONS.md`. |
